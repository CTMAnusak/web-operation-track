...
- แสดงรายการหัตถการเป็นการ์ด; แตะเปิด **ProcDetailPopup** แก้ไข/บันทึก/จบหัตถการ (การลบการ์ดและโหมดแก้ไขหลังเสร็จสิ้น — ดูหัวข้อถัดไป)  
...
#### กฎการ์ดหัตถการ — สถานะเสร็จสิ้น และปุ่มลบ

| หัวข้อ | พฤติกรรมในโค้ด |
|--------|------------------|
| ปุ่มลบ (กากบาท) | เฉพาะ role พยาบาล / ผู้ช่วยแพทย์ / นักกายภาพ (`r2`, `r3`, `r4`) และเฉพาะหัตถการที่สถานะ **ยังไม่เสร็จสิ้น** — การ์ดสถานะ **เสร็จสิ้น** **ไม่แสดงปุ่มลบ** |
| แก้ไขหลังเสร็จสิ้น | เปิดรายละเอียดได้เสมอ; ถ้าสถานะเสร็จสิ้นและเวลาปัจจุบันห่างจาก `createdAt` ของรายการ **ไม่เกิน 24 ชั่วโมง** → แก้ไขใน **ProcDetailPopup** ได้ |
| โหมดอ่านอย่างเดียว | ถ้าสถานะเสร็จสิ้นและผ่าน `createdAt` ไป **มากกว่า 24 ชั่วโมง** → `readOnly` (ดูได้ แก้ไม่ได้) |

ช่วง 24 ชม. นับจาก **`createdAt`** ของแต่ละหัตถการ (เวลาที่สร้างรายการในระบบ) ไม่ได้แยก track “ครั้งแรกที่เปิด popup” เป็นฟิลด์ต่างหาก

ฟังก์ชันอ้างอิงใน `OpdDetailPage.jsx`: `canDeleteProcCard`, `isCardOlderThanOneDay`

#### ProcDetailPopup — การตรวจฟอร์ม, ปุ่มบันทึก และปุ่มเสร็จสิ้นหัตถการ

คอมโพเนนต์ **ProcDetailPopup** ใน `OpdDetailPage.jsx` เป็นหน้าเต็มจอเดียวกันสำหรับทุกประเภทหัตถการ (เครื่อง / ฉีด / Wellness / Laser)

| รายการ | พฤติกรรม |
|--------|----------|
| ช่อง **รายละเอียดเพิ่มเติม** (`note`) | **ไม่บังคับ** — กรอกหรือไม่กรอกไม่กระทบความถูกต้องของฟอร์ม |
| เงื่อนไขครบฟอร์ม | รวมอยู่ในตัวแปร `isFormValid` (เช่น ตำแหน่งร่างกายสำหรับเครื่อง–ฉีด–เลเซอร์, การตั้งค่าเครื่องหรือเลเซอร์ตามกฎในโค้ด, มีผู้ร่วมทำหัตถการอย่างน้อยหนึ่งคน, ฟิลด์ Slim pen / IV Drip ครบตามประเภท Wellness) |
| ปุ่ม **บันทึก** | `handleSave` — ตั้ง `saveAttempted` เป็น `true`; ถ้า `!isFormValid` จะไม่เรียก `onSave` และแสดงขอบ/สไตล์ error บนช่องที่ขาด (คลาส `--error` / `proc-popup__section--error` ฯลฯ) |
| ปุ่ม **เสร็จสิ้นหัตถการ** | `handleRequestComplete` — ใช้ **เกณฑ์เดียวกับบันทึก** (`isFormValid` + `saveAttempted` เพื่อแสดงขอบแดง); ถ้าข้อมูลไม่ครบจะ**ไม่**เปิด dialog; ถ้าครบแล้วค่อยแสดง **ยืนยันการเสร็จสิ้นหัตถการ** (`ConfirmDialog`) ก่อนเรียก `onComplete` |

#### Wellness — Slim pen: การคำนวณเปอร์เซ็นต์ในการ์ด «น้ำหนักเป้าหมาย»

บน **ProcDetailPopup** การ์ดน้ำหนักเป้าหมายแสดงตัวเลขเปอร์เซ็นต์ที่ป้าย **ลดสำเร็จแล้ว** และวงกลมความคืบหน้า (ค่าเดียวกัน) โดยคำนวณใน `OpdDetailPopup` เป็นค่า **ระดับความใกล้เคียงเป้าหมาย (%)** จากข้อมูลในฟอร์มเท่านั้น **ไม่อิงประวัติย้อนหลัง** ในสูตรนี้

**ตัวแปรที่ใช้**

| ตัวแปร | ความหมาย | ฟิลด์ใน procedure |
|--------|-----------|-------------------|
| น้ำหนักปัจจุบัน | กก. | `weight` |
| น้ำหนักเป้าหมาย | กก. | `targetWeight` |
| เปอร์เซ็นต์ไขมัน | % | `fat` |
| มวลกล้ามเนื้อ | กก. | `muscle` |

**ขั้นตอนคำนวณ**

1. **Weight Score** = (น้ำหนักเป้าหมาย ÷ น้ำหนักปัจจุบัน) × 100  
2. **Fat Score** = 100 − เปอร์เซ็นต์ไขมัน  
3. **Muscle Score** = (มวลกล้ามเนื้อ ÷ น้ำหนักปัจจุบัน) × 100  
4. จำกัด **Weight / Fat / Muscle Score** แต่ละค่าให้อยู่ในช่วง **0–100**  
5. **ผลรวมถ่วงน้ำหนัก** = (Weight Score × 0.5) + (Fat Score × 0.25) + (Muscle Score × 0.25)  
6. จำกัดผลรวมให้อยู่ในช่วง **0–100** แล้วแสดงเป็น**จำนวนเต็ม** (ปัดเศษแบบ `Math.round`)

**กรณีได้ 0%**

- น้ำหนักปัจจุบัน ≤ 0 หรือค่าใดค่าหนึ่งในสี่ตัวแปรด้านบนแปลงเป็นตัวเลขไม่ได้ (`NaN`)

**หมายเหตุ:** ฟิลด์ `startWeight` อาจยังถูกตั้งตอนบันทึกหัตถการ (baseline สำหรับข้อมูลเก่า/การเก็บ state) แต่**ไม่นำมาใช้**ในสูตรเปอร์เซ็นต์ข้างต้น
...
*อัปเดตตามสภาพรีโป มีนาคม 2026 — หากมีการเพิ่ม API หรือย้ายข้อมูลออกจาก mock ควรปรับส่วน “สถานะการเชื่อมต่อ Backend” ให้สอดคล้อง*
