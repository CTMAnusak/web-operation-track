...
/* ─────────────────────────────────────────
   QR Scanner Modal
───────────────────────────────────────── */
function QRScannerModal({ onClose, onScan, mockHN = 'OPDX32434', title = 'ค้นหาผู้ร่วมทำหัตถการ' }) {
  ...
  }, [cameraReady, scanning, onScan, mockHN]);
  ...
  return (
    <div className="qr-modal">
      ...
      <div className="qr-modal__body">
        <h2 className="qr-modal__title">{title}</h2>
...
function OpdDetailPage() {
  ...
  function handleHeaderQRScan(hn) {
    setScannedHN(hn);
    setShowHeaderQR(false);
  }

  function clearScannedHN() {
    setScannedHN('');
  }
  ...
  /* if showing header QR scanner */
  if (showHeaderQR) {
    return (
      <MobileLayout>
        <QRScannerModal
          onClose={() => setShowHeaderQR(false)}
          onScan={handleHeaderQRScan}
          mockHN="62173489"
          title="ค้นหา HN ผู้ป่วย"
        />
      </MobileLayout>
    );
  }
      ...
      <div className="opd-detail">
        ...
        <header className="opd-detail__topbar">
          ...
          <div className="opd-detail__topbar-actions">
            {scannedHN ? (
              <div className="opd-detail__scanned-hn">
                <span>{scannedHN}</span>
                <button
                  className="opd-detail__clear-hn-btn"
                  onClick={clearScannedHN}
                  aria-label="ล้างเลข HN"
                >
                  <IconClose />
                </button>
              </div>
              ...
              <>
                ...
                <button
                  className="opd-detail__topbar-icon-btn"
                  aria-label="QR Code"
